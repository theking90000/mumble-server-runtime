...
<head>
  ...
  <style>
    :root { color-scheme: dark; --bg:#07111f; --panel:#0c192b; --panel2:#102139; --text:#eaf2ff; --muted:#93a9c5; --line:#233958; --cyan:#50d5ff; --violet:#a78bfa; --green:#42d392; --amber:#f7bd62; --red:#ff6b7a; }
    * { box-sizing:border-box; }
    body { margin:0; background:radial-gradient(circle at 20% -10%,#163b59 0,transparent 32rem),var(--bg); color:var(--text); font:14px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
    main { width:min(1480px,calc(100% - 32px)); margin:0 auto; padding:36px 0 64px; }
    h1,h2,h3,p { margin-top:0; } h1 { font-size:clamp(30px,4vw,54px); letter-spacing:-.04em; margin-bottom:8px; } h2 { font-size:20px; margin-bottom:16px; } h3 { font-size:14px; color:var(--muted); font-weight:600; text-transform:uppercase; letter-spacing:.08em; }
    .eyebrow { color:var(--cyan); font-weight:750; letter-spacing:.12em; text-transform:uppercase; }
    ...
    .status { min-width:220px; border:1px solid var(--line); border-radius:18px; padding:16px 18px; background:rgba(12,25,43,.82); box-shadow:0 18px 60px rgba(0,0,0,.22); }
    .status strong { display:block; font-size:17px; letter-spacing:.04em; } .status span { color:var(--muted); }
    .status.pass,.status.partial { border-color:rgba(66,211,146,.45); } .status.pass strong,.status.partial strong { color:var(--green); }
    .status.fail,.status.incomplete { border-color:rgba(255,107,122,.5); } .status.fail strong,.status.incomplete strong { color:var(--red); }
    .cards { display:grid; grid-template-columns:repeat(6,minmax(140px,1fr)); gap:12px; margin-bottom:24px; }
    .card,.panel { background:linear-gradient(145deg,rgba(16,33,57,.96),rgba(10,23,40,.96)); border:1px solid var(--line); border-radius:16px; box-shadow:0 16px 50px rgba(0,0,0,.16); }
    .card { padding:16px; min-height:112px; } .card .label { color:var(--muted); font-size:12px; text-transform:uppercase; letter-spacing:.07em; } .card .value { display:block; font-size:27px; font-weight:760; margin-top:7px; letter-spacing:-.03em; } .card .hint { color:var(--muted); font-size:12px; }
    .grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:16px; margin-bottom:24px; } .panel { padding:20px; min-width:0; } .wide { grid-column:1/-1; }
    .chart { min-height:285px; } svg { width:100%; height:auto; display:block; overflow:visible; } .axis { stroke:var(--line); stroke-width:1; } .gridline { stroke:var(--line); stroke-width:1; opacity:.65; } .tick { fill:var(--muted); font-size:11px; } .legend { display:flex; flex-wrap:wrap; gap:14px; color:var(--muted); margin:-4px 0 12px; font-size:12px; } .legend i { width:9px; height:9px; border-radius:50%; display:inline-block; margin-right:6px; }
    table { width:100%; border-collapse:collapse; } th,td { padding:10px 12px; border-bottom:1px solid var(--line); text-align:left; white-space:nowrap; } th { color:var(--muted); font-size:11px; letter-spacing:.07em; text-transform:uppercase; } td:first-child { white-space:normal; } tr:last-child td { border-bottom:0; }
    .scroll { overflow-x:auto; } .badge { display:inline-flex; align-items:center; border-radius:999px; padding:3px 8px; font-size:11px; font-weight:750; letter-spacing:.04em; text-transform:uppercase; } .badge.pass { color:var(--green); background:rgba(66,211,146,.12); } .badge.fail { color:var(--red); background:rgba(255,107,122,.12); } .badge.unknown { color:var(--amber); background:rgba(247,189,98,.12); }
    .notice { padding:12px 14px; border-left:3px solid var(--amber); background:rgba(247,189,98,.08); color:#f8d99f; border-radius:4px 10px 10px 4px; margin:10px 0; }
    .failure { border-left-color:var(--red); background:rgba(255,107,122,.08); color:#ffc2c9; }
    .meta { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:1px; border:1px solid var(--line); background:var(--line); border-radius:12px; overflow:hidden; } .meta div { background:var(--panel); padding:12px; } .meta dt { color:var(--muted); font-size:11px; text-transform:uppercase; letter-spacing:.06em; } .meta dd { margin:3px 0 0; overflow-wrap:anywhere; }
    .artifacts { display:flex; gap:8px; flex-wrap:wrap; } a { color:var(--cyan); text-decoration:none; } a:hover { text-decoration:underline; } .artifact { border:1px solid var(--line); background:var(--panel2); padding:7px 10px; border-radius:8px; }
    ...
    @media (max-width:720px) { main { width:min(100% - 20px,1480px); padding-top:22px; } .hero { display:block; } .status { margin-top:18px; } .grid { grid-template-columns:1fr; } .wide { grid-column:auto; } .cards { grid-template-columns:repeat(2,1fr); } .meta { grid-template-columns:1fr; } }
  </style>
</head>
<body><main>
...
const colors=['#50d5ff','#a78bfa','#42d392','#f7bd62','#ff6b7a','#5eead4'];
...
<style>:root{color-scheme:dark;--bg:#07111f;--panel:#102139;--line:#233958;--text:#eaf2ff;--muted:#93a9c5;--cyan:#50d5ff;--green:#42d392;--amber:#f7bd62;--red:#ff6b7a}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 15% -10%,#163b59,transparent 32rem),var(--bg);color:var(--text);font:14px/1.5 Inter,system-ui,sans-serif}main{width:min(1500px,calc(100% - 32px));margin:auto;padding:38px 0 70px}h1{font-size:clamp(34px,5vw,58px);letter-spacing:-.045em;margin:0}.eyebrow{color:var(--cyan);text-transform:uppercase;letter-spacing:.12em;font-weight:750}.muted{color:var(--muted)}.cards{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin:28px 0}.card,.panel{background:linear-gradient(145deg,rgba(16,33,57,.97),rgba(10,23,40,.97));border:1px solid var(--line);border-radius:16px}.card{padding:16px}.card span{display:block;color:var(--muted);font-size:12px;text-transform:uppercase}.card strong{font-size:30px}.panel{padding:20px}.scroll{overflow:auto}table{width:100%;border-collapse:collapse}th,td{padding:11px 12px;border-bottom:1px solid var(--line);text-align:left;white-space:nowrap}th{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.07em;cursor:pointer}a{color:var(--cyan);text-decoration:none}.badge{padding:3px 8px;border-radius:999px;font-size:11px;font-weight:750}.pass,.partial{color:var(--green);background:rgba(66,211,146,.12)}.fail,.incomplete{color:var(--red);background:rgba(255,107,122,.12)}@media(max-width:800px){.cards{grid-template-columns:repeat(2,1fr)}}</style></head>
<body><main><div class="eyebrow">Spaces headless load campaign</div><h1 id="title"></h1><p class="muted">Standalone reports generated from the artifacts already on disk.</p><section class="cards" id="cards"></section><section class="panel"><div class="scroll"><table><thead><tr><th>Run</th><th>Status</th><th>Scenario</th><th>Participants</th><th>Per Space</th><th>Controllers</th><th>Sync p99</th><th>Max queue</th><th>Peak connections</th><th>Elapsed</th></tr></thead><tbody id="runs"></tbody></table></div></section></main>
<script id="campaign-data" type="application/json">__DATA__</script><script>const data=JSON.parse(document.getElementById('campaign-data').textContent);const el=(n,t,c)=>{const x=document.createElement(n);if(t!=null)x.textContent=t;if(c)x.className=c;return x};const fmt=(v,d=0)=>v==null?'n/a':Number(v).toLocaleString(undefined,{maximumFractionDigits:d});document.getElementById('title').textContent=data.root_name;const counts={runs:data.runs.length,pass:0,partial:0,fail:0,incomplete:0};data.runs.forEach(r=>counts[r.status.code]++);for(const [label,value] of [['Runs',counts.runs],['Pass',counts.pass],['Partial pass',counts.partial],['Issues',counts.fail],['Incomplete',counts.incomplete]]){const c=el('div',null,'card');c.append(el('span',label),el('strong',value));document.getElementById('cards').append(c)}for(const run of data.runs){const row=el('tr');const link=el('a',run.run_name);link.href=run.report_href;const first=el('td');first.append(link);const badge=el('span',run.status.label,'badge '+run.status.code);const status=el('td');status.append(badge);row.append(first,status,el('td',run.scenario),el('td',fmt(run.participants)),el('td',fmt(run.participants_per_space)),el('td',fmt(run.controllers)),el('td',run.sync_p99==null?'n/a':fmt(run.sync_p99,1)+' ms'),el('td',fmt(run.max_queue)),el('td',fmt(run.max_connections)),el('td',run.elapsed_millis==null?'n/a':fmt(run.elapsed_millis/1000,1)+' s'));document.getElementById('runs').append(row)}</script></body></html>'''
